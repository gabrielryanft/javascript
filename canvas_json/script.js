...
class particle {
  constructor() {
    ...
    for (let i = 0; i < hex_str.length; i++) {
      if (
        !["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].includes(hex_str[i])
      ) {
      ...
      if (r >= this.variation_col && r <= (255 - this.variation_col)) {
        var some_num = Math.random() * (this.variation_col * 2) -
          this.variation_col;
      } else if (r < this.variation_col) {
        var some_num = Math.random() * (this.variation_col - (r * -1)) +
          (r * -1);
      } else {
        var some_num = Math.random() * (this.variation_col + (255 - r)) -
          this.variation_col;
      }
      if (g >= this.variation_col && g <= (255 - this.variation_col)) {
        ...
        var some_num = Math.random() * (this.variation_col - (g * -1)) +
          (g * -1);
      } else {
        var some_num = Math.random() * (this.variation_col + (255 - g)) -
          this.variation_col;
      }
      if (b >= this.variation_col && b <= (255 - this.variation_col)) {
        ...
        var some_num = Math.random() * (this.variation_col - (b * -1)) +
          (b * -1);
      } else {
        var some_num = Math.random() * (this.variation_col + (255 - b)) -
          this.variation_col;
      }
      if (this.variation_col == "0") {
        this.color = `rgba(${r}, ${g}, ${b}, ${this.opacity})`;
      } else {
        this.color = `rgba(${Math.round(r + some_num)}, ${Math.round(g + some_num)
          }, ${Math.round(b + some_num)}, ${this.opacity})`;
      }
    }
    ...
    this.size = part_sz *
      ((Math.random() *
        ((1 + this.size_variation) - (1 - this.size_variation))) +
        (1 - this.size_variation)); //(math.random * (max-min)) + min
  }
  draw() {
    cc.beginPath();
    cc.fillStyle = this.color;
...
<span class="indent_2"></span><span class="key">c</span>: <span class="str">"${instructions[i].c
      }"</span>,<br>
<span class="indent_2"></span><span class="key">o_c</span>: <span class="str">"${instructions[i].o_c
      }"</span>,<br>
<span class="indent_2"></span><span class="key">c_v</span>: <span class="num">${instructions[i].c_v
      }</span>,<br>
<span class="indent_2"></span><span class="key">o</span>: <span class="num">${instructions[i].o
      }</span>,<br>
<span class="indent_2"></span><span class="key">o_b_s</span>: <span class="num">${instructions[i].o_b_s
      }</span>, <br>
<span class="indent_2"></span><span class="key">b_s</span>: <span class="num">${instructions[i].b_s
      }</span>, <br>
<span class="indent_2"></span><span class="key">s_v</span>: <span class="num">${instructions[i].s_v
      }</span>, <br>
<span class="indent_2"></span><span class="key">i_g</span>: <span class="num">${instructions[i].i_g
      }</span>, <br>
<span class="indent_2"></span><span class="key">i_s</span>: <span class="num">${instructions[i].i_s
      }</span>, <br>
<span class="indent_2"></span><span class="key">i_a</span>: <span class="num">${instructions[i].i_a
      }</span>, <br>
<span class="indent_2"></span><span class="key">x</span>: <span class="num">${instructions[i].x
      }</span>, <br>
<span class="indent_2"></span><span class="key">y</span>: <span class="num">${instructions[i].y
      }</span><br>
<span class="indent_1"></span><span class="crl_bkts">}</span>,<br>`;
  }
  ...
  for (let i = 0; i < (instructions_sctchs_pts.length); i++) {
    list_sctchs_pts +=
      '<span class="indent_1"></span><span class="sqr_bkts">[</span><br>';
    for (let ii = 0; ii < instructions_sctchs_pts[i].length; ii++) {
      list_sctchs_pts += `
<span class="indent_2"></span><span class="crl_bkts">{</span><br>
<span class="indent_3"></span><span class="key">c</span>: <span class="str">"${instructions_sctchs_pts[i][ii].c
        }"</span>,<br>
<span class="indent_3"></span><span class="key">o_c</span>: <span class="str">"${instructions_sctchs_pts[i][ii].o_c
        }"</span>,<br>
<span class="indent_3"></span><span class="key">c_v</span>: <span class="num">${instructions_sctchs_pts[i][ii].c_v
        }</span>,<br>
<span class="indent_3"></span><span class="key">o</span>: <span class="num">${instructions_sctchs_pts[i][ii].o
        }</span>,<br>
<span class="indent_3"></span><span class="key">o_b_s</span>: <span class="num">${instructions_sctchs_pts[i][ii].o_b_s
        }</span>, <br>
<span class="indent_3"></span><span class="key">b_s</span>: <span class="num">${instructions_sctchs_pts[i][ii].b_s
        }</span>, <br>
<span class="indent_3"></span><span class="key">s_v</span>: <span class="num">${instructions_sctchs_pts[i][ii].s_v
        }</span>, <br>
<span class="indent_3"></span><span class="key">i_g</span>: <span class="num">${instructions_sctchs_pts[i][ii].i_g
        }</span>, <br>
<span class="indent_3"></span><span class="key">i_s</span>: <span class="num">${instructions_sctchs_pts[i][ii].i_s
        }</span>, <br>
<span class="indent_3"></span><span class="key">i_a</span>: <span class="num">${instructions_sctchs_pts[i][ii].i_a
        }</span>, <br>
<span class="indent_3"></span><span class="key">x</span>: <span class="num">${instructions_sctchs_pts[i][ii].x
        }</span>, <br>
<span class="indent_3"></span><span class="key">y</span>: <span class="num">${instructions_sctchs_pts[i][ii].y
        }</span><br>
<span class="indent_2"></span><span class="crl_bkts">}</span>,<br>
`;
      if (ii == (instructions_sctchs_pts[i].length - 1)) {
        ...
        list_sctchs_pts +=
          '<br><span class="indent_1"></span><span class="sqr_bkts">]</span>,<br>';
      }
    }
  }
...
const cf = document.querySelector("#footer-canvas");

const cfc = cf.getContext("2d");

let mouse_over;

let widthCanvasFooter = cf.getBoundingClientRect().width;
let heightCanvasFooter = cf.getBoundingClientRect().height;
cf.width = widthCanvasFooter;
cf.height = heightCanvasFooter;

window.addEventListener("resize", () => {
  widthCanvasFooter = cf.getBoundingClientRect().width;
  heightCanvasFooter = cf.getBoundingClientRect().height;
  cf.width = widthCanvasFooter;
  cf.height = heightCanvasFooter;
});
...
cf.addEventListener("pointermove", () => {
  cfc.beginPath();
  cfc.fillStyle = `rgb(${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 255)
    }, ${Math.floor(Math.random() * 255)})`;
  cfc.rect(
    Math.floor(Math.random() * widthCanvasFooter) - (20 / 2),
    Math.floor(Math.random() * heightCanvasFooter) - (20 / 2),
    20,
    20,
  );
  cfc.fill();
